...
class ssEnvCheck {
	function ssEnvCheck() {
		...
		// Check PHP version
		if (!get_option('ss_ignorephpver') && ((PHP_VERSION_ID > 50999) || (PHP_VERSION_ID < 50300))) {
		...
		if (!@$this->sspro && !isset($this->haserror) && !get_option('ss_ignoresspro') && !isset($_GET['ss_ignoresspro'])) {
			add_action('admin_notices',array($this,'lite_update_notice'));
		}
		...
		if (@$this->sspro) {
			...
			add_action('admin_notices',array($this,'error_notice_stored'));
		}
	}
...
?>
	...
	function lite_update_notice() {
?>
<div class="notice notice-info">
<p><table cellspacing="0" cellpadding="0" border="0"><tr><td style="padding-right:10px;"><span class="dashicons dashicons-yes" style="width:40px;height:40px;font-size:40px;"></span></td><td>Upgrade to <a href="http://www.symbiostock.org/shop/" target="_blank"><B>Symbiostock Pro</b></a> for <B>free</b>! Custom watermarks, FTP uploads, licensing, vectors and more are just some of the advanced features available. For more information about the versioning since version 3.2 go <a href="http://www.symbiostock.org/docs/pro-vs-lite/" target="_blank">here</a>. Click <a href="<?php print admin_url( 'edit.php?post_type=product&ss_ignoresspro=1'); ?>">here</a> to ignore</td></tr></table></p>
</div>
<?php
	}
	...
	function error_notice_phpver() {
		...
		<div class="update-nag">
			<?php print __('Symbiostock has been verified with PHP 5.3 to 5.6. You are currently running version '.phpversion().'. We suggest changing to a verified version to avoid bugs. Click <a href="'.admin_url( 'edit.php?post_type=product&ss_ignorephpver=1').'">here</a> to ignore this warning.','ss'); ?>
		</div>
		<?php
	}
...
if (isset($_GET['ss_purgerrs'])) {
	ss_update_option('ss_errs',array());
	header("Location: ".admin_url( 'edit.php?post_type=product'));
	exit();
}

if ($_GET["page"] == 'ss_updates') {
